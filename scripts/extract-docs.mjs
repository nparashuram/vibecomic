...
const CONVENTIONS = [
  ...
  '  device). A save checks Drive first: if somebody else saved since the project was opened,',
...
const DATA_MODEL = [
  ...
  '- `ComicPage`: `{ id, number, title, prompt?, panels[] }`: `number` is 0-based and shown as 0, 1, …; `prompt` is the intent of the page (stitched first into the image prompt of every layer on it)',
  '- `Panel`: `{ id, title?, prompt?, x, y, width, height, layers[], bubbles[] }`: `x`/`y`/`width`/`height` are % of the page; the panels of a page tile it with no gaps; `prompt` is the intent of the panel (stitched after the page prompt, before the layer prompt)',
